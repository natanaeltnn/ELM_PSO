...
if __name__ == '__main__':
	...
	for dia in datas:
		...
		print(dia)
		old = [nome for nome in os.listdir('../Open/'+dia+"-horario")]
		original = copy.copy(old)
		new = []
		while num_alcancado < num_ativos:

			for nova_acao in old:
				pred_values = do_prediction('../historico_horario/',4)
				print('novo horario: ',nova_acao)
				nome = nova_acao

				nome_ativo = pred_values[0][0]
				#print(nome_ativo)
				predicoes = copy.copy(pred_values[0][1:])

				for i in range(num_rodadas):
					x_sol,ps,op = func_CPSO(nova_acao+"/IND$",predicoes,i,dia,1)
			...
			if not old == []:
				...
				registrar_best(best_pred,'Predicoes/ind_horario_'+dia,dia,num_rodadas,-1,media=False)

			original = original+new
			new = [nome for nome in os.listdir('../Open/'+dia+"-horario")]
